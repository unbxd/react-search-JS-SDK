...
class GenerateFacets extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.bucketedFacets !== bucketedFacets) {
      const formattedBucketedFacets = bucketedFacets.map((bucketedFacet) => {
        const matchBucketedFacet = this.state.bucketedFacets.find(
          (bucketedFacetObj) => {
            return bucketedFacetObj.displayName === bucketedFacet.displayName;
          }
        );
        return {
          ...bucketedFacet,
          isOpen: matchBucketedFacet ? matchBucketedFacet.isOpen : true,
          filter: matchBucketedFacet ? matchBucketedFacet.filter : ''
        };
      });

      this.setState((currentState) => {
        return { ...currentState, bucketedFacets: formattedBucketedFacets };
      });
    }
  }
      ...
      const updatedTextFacets = currentState.bucketedFacets.map(
        (bucketedFacet) => {
          if (facetId === bucketedFacet.displayName) {
            return { ...bucketedFacet, isOpen: !bucketedFacet.isOpen };
          }
  ...
  handleFilterChange(event) {
    ...
    this.setState((currentState) => {
      const updatedTextFacets = currentState.bucketedFacets.map(
        (bucketedFacet) => {
          if (facetId === bucketedFacet.displayName) {
            return { ...bucketedFacet, filter: value.toLowerCase() };
          }
          return { ...bucketedFacet };
        }
      );
